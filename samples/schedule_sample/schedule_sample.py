...
Walks through the full schedule lifecycle against a running Cadence server:
    ...
    create → describe → pause → unpause → backfill → list → update → delete
...
async def run_demo(args: argparse.Namespace) -> None:
    ...
    print(f"Describe : cron={desc.spec.cron_expression!r}  paused={desc.state.paused}")
    ...
    # --- backfill (2 triggers in the last 2 hours) ---
