...
async def test_worker():
    ...
    worker_stub.PollForDecisionTask.assert_called_once_with(PollForDecisionTaskRequest(
        domain="domain",
        identity="identity",
        task_list=TaskList(name="task_list", kind=TaskListKind.TASK_LIST_KIND_NORMAL),
    ), timeout=60.0)

    worker_stub.PollForActivityTask.assert_called_once_with(PollForActivityTaskRequest(
        domain="domain",
        identity="identity",
        task_list=TaskList(name="task_list", kind=TaskListKind.TASK_LIST_KIND_NORMAL),
    ), timeout=60.0)